...
def rect_slot_ul(x, y, w, h, com=True, chain=True, leadin='g0'):
    if com:
        line()
        line('(rect_slot_ul X%s Y%s W%s H%s)' %
...
def rect_in_ul(x, y, w, h, finishes=1, chain=True, com=True):
    ...
    if finishes:
        ...
        line('(Rough)')
        rect_slot_ul(x + cnc.em / 2 + cnc.finish_u,
                     y + cnc.em / 2 + cnc.finish_u,
                     w - cnc.em - cnc.finish_u,
                     h - cnc.em - cnc.finish_u,
                     com=False,
                     chain=False)
        # Finishing pass
        line('(Finish)')
        rect_slot_ul(x + cnc.em / 2,
                     y + cnc.em / 2,
                     w - cnc.em,
                     h - cnc.em,
                     com=False,
...
def rect_in_cent(x, y, w, h, *args, **kwargs):
    ...
    if kwargs.get('com', True):
        line()
        line('(rect_in_cent X%s Y%s W%s H%s)' %
             (fmt(x), fmt(y), fmt(w), fmt(h)))
    kwargs['com'] = False
